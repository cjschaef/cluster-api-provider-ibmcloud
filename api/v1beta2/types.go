...
// +kubebuilder:validation:XValidation:rule="self.direction == 'inbound' && !has(self.destination)",message="destination is not valid for SecurityGroupRuleDirectionInbound direction"
// +kubebuilder:validation:XValidation:rule="self.direction == 'outbound' && !has(self.source)",message="source is not valid for SecurityGroupRuleDirectionOutbound direction"
...
// +kubebuilder:validation:XValidation:rule="self.remoteType == 'cidr' && has(self.cidrSubnetName) && !has(self.ip) && !has(self.securityGroupName)",message="cidrSubnetName is only valid for SecurityGroupRuleRemoteTypeCIDR remoteType"
// +kubebuilder:validation:XValidation:rule="self.remoteType == 'ip' && has(self.ip) && !has(self.cidrSubnetName) && !has(self.securityGroupName)",message="ip is only valid for SecurityGroupRuleRemoteTypeIP remoteType"
// +kubebuilder:validation:XValidation:rule="self.remoteType == 'sg' && has(self.securityGroupName) && !has(self.cidrSubnetName) && !has(self.ip)",message="securityGroupName is only valid for SecurityGroupRuleRemoteTypeSG remoteType"
...
// +kubebuilder:validation:XValidation:rule="(self.protocol != 'icmp' && !has(self.icmpCode) && !has(self.icmpType)) || self.protocol == 'icmp'",message="icmpCode and icmpType are only supported for the ICMP protocol"
// +kubebuilder:validation:XValidation:rule="self.protocol == 'icmp' && !has(self.portRange)",message="portRange is not valid for SecurityGroupRuleProtocolIcmp protocol"
